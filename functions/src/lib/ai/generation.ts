...
export function evaluatePlatformFit(contentDna: ContentDNA, platform: PlatformProfile): number {
  ...
  for (const suggestion of contentDna.suggestedPlatforms || []) {
    ...
    if (pid === platform.platformId || platform.platformId.includes(pid)) {
      if (suggestion.fitScore > 0.7) score += 0.05;
      break;
    }
  }
...
function buildGenerationPrompt(
  ...
  if (voiceProfile) {
    ...
    const banned = (vocab as Record<string, unknown>).banned_terms as string[] ?? [];
    const preferred = (vocab as Record<string, unknown>).preferred_terms as string[] ?? [];
...
Approved Topics: ${((topics as Record<string, unknown>).approved_topics as string[] ?? []).join(", ") || "No restrictions"}
Restricted Topics (AVOID): ${((topics as Record<string, unknown>).restricted_topics as string[] ?? []).join(", ") || "None"}`;
  } else {
    voiceSection = `
## Creator's Voice
